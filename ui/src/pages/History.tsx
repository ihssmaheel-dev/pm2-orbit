...
export function History() {
  ...
  useEffect(() => {
    if (selectedProcessId === null) { setProcessHistory([]); return; }
    const hours = timeRange === '1h' ? 1 : timeRange === '6h' ? 6 : 24;
  ...
  const cpuData = useMemo(() => ({ ts: systemHistory.map((r) => r.ts), values: systemHistory.map((r) => r.cpu) }), [systemHistory]);
  const memData = useMemo(() => ({ ts: systemHistory.map((r) => r.ts), values: systemHistory.map((r) => r.memoryTotal > 0 ? Math.round((r.memoryUsed / r.memoryTotal) * 1000) / 10 : 0) }), [systemHistory]);
  const loadData = useMemo(() => ({ ts: systemHistory.map((r) => r.ts), load1: systemHistory.map((r) => r.load1), load5: systemHistory.map((r) => r.load5), load15: systemHistory.map((r) => r.load15) }), [systemHistory]);
  const processCpuData = useMemo(() => ({ ts: processHistory.map((r) => r.ts), values: processHistory.map((r) => r.cpu) }), [processHistory]);
  const processMemData = useMemo(() => ({ ts: processHistory.map((r) => r.ts), values: processHistory.map((r) => r.memory ? Math.round(r.memory / (1024 * 1024) * 10) / 10 : 0) }), [processHistory]);
        ...
        <div className="ml-auto flex items-center gap-1">
          {(['1h', '6h', '24h'] as TimeRange[]).map((range) => (
            <button key={range} onClick={() => setTimeRange(range)}
              className={`px-2.5 py-1 text-[11px] font-medium uppercase tracking-wider transition-colors cursor-pointer border border-border/60 ${timeRange === range ? 'bg-primary/10 text-primary border-primary/40' : 'text-muted-foreground hover:text-foreground hover:border-border/80'}`}>
              {range}
            </button>
          ))}
        </div>
          ...
          {systemLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="bg-card border border-border p-4 h-[200px] flex items-center justify-center">
                  <span className="text-xs text-muted-foreground/50">Loading...</span>
                </div>
              ))}
            </div>
          ) : systemError ? (
            <div className="flex items-center justify-center h-[160px] bg-card border border-destructive/20 gap-2">
              <AlertCircle size={14} className="text-destructive" />
              <span className="text-xs text-destructive">{systemError}</span>
            </div>
          ) : systemHistory.length === 0 ? (
            <div className="flex items-center justify-center h-[160px] bg-card border border-border gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="bg-card border border-border p-4 flex flex-col">
                ...
                <div className="h-[160px]"><CpuChart data={cpuData} /></div>
              </div>
              <div className="bg-card border border-border p-4 flex flex-col">
                <div className="flex items-center justify-between mb-3">
                ...
                <div className="h-[160px]"><MemoryChart data={memData} /></div>
              </div>
              <div className="bg-card border border-border p-4 flex flex-col">
                <div className="flex items-center justify-between mb-3">
                ...
                <div className="h-[160px]"><LoadChart data={loadData} /></div>
              </div>
            </div>
          )}
          {lastSys && (
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="relative">
              <select value={selectedProcessId ?? ''} onChange={(e) => setSelectedProcessId(e.target.value ? parseInt(e.target.value, 10) : null)}
                ...
                {processList.map((p) => (<option key={p.id} value={p.id}>{p.name} (PID {p.pid})</option>))}
              </select>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="bg-card border border-border p-4 flex flex-col">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {processHistory.length > 0 && (
                      <div className="flex items-center gap-3 text-[10px] text-muted-foreground/60">
                        <span>Now: <span className="text-foreground/80 font-mono">{fmtPercent(processHistory[processHistory.length - 1].cpu)}</span></span>
                        <span>Peak: <span className="text-foreground/80 font-mono">{fmtPercent(Math.max(...processHistory.map((r) => r.cpu)))}</span></span>
                      </div>
                    )}
                  </div>
                  <div className="h-[160px]"><CpuChart data={processCpuData} /></div>
                </div>
                <div className="bg-card border border-border p-4 flex flex-col">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-medium">Memory (MB)</span>
                    {processHistory.length > 0 && (
                      <div className="flex items-center gap-3 text-[10px] text-muted-foreground/60">
                        <span>Now: <span className="text-foreground/80 font-mono">{processHistory[processHistory.length - 1].memory} bytes</span></span>
                        <span>Peak: <span className="text-foreground/80 font-mono">{formatBytes(Math.max(...processHistory.map((r) => r.memory)))}</span></span>
                      </div>
                    )}
                  </div>
